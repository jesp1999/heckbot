...
from src.cogs import ai, reactmatch, poll, gif, sfx
...
async def on_ready() -> None:
    """
    Event listener triggered when the bot is authenticated and ready to run. Sets up all cogs for the application,
    notifies the guilds it is present in about its online status, and updates its presence.
    """
...
async def on_member_join(member: discord.Member) -> None:
    """
    Event listener triggered when the bot detects a new member joining the server. Sends a message in whitelisted
    servers with a designated welcome channel to welcome the user to the server.
