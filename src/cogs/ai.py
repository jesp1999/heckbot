...
class AI(commands.Cog):
    ...
    async def imagine(self, ctx: Context, *prompt_terms) -> None:
        ...
        self._user_image_cache[
            ImageCacheEntry(user=ctx.author.id, channel=ctx.channel.id,
                            guild=ctx.guild.id)] = image_url

        print(image_url)
        await ctx.send(image_url)

    @commands.command()
    async def reimagine(self, ctx: Context) -> None:
        image_url = self._user_image_cache.get(
            ImageCacheEntry(user=ctx.author.id, channel=ctx.channel.id,
                            guild=ctx.guild.id))
